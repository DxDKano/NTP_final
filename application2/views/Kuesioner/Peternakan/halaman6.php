					...
					<div class="card-body">
						...
						<table class="table table-sm table-bordered small">
							...
							<tr>
								...
								<td class="table-secondary" colspan="2"><input id="b4d_k6_total" type="number" class="form-control" value="<?php echo $sub['b4d_k6_total']?>" ></td>
							...
							<tr>
								<td class="table-secondary" colspan="10" >E. Barang Modal</td>
								<td class="table-secondary" colspan="2"><input id="b4e_k6_total" type="number" class="form-control" value="<?php echo $sub['b4e_k6_total']?>" ></td>
								<td class="table-secondary" colspan="1"></td>
							</tr>
							<?php echo $isianBlok4e; ?>
							<tr id="tr_tambah_b4_e">
								<td colspan="13" >
									<button type="button" class="btn btn-outline-secondary" id="btn_tambah_b4_e">
										<i class="bi bi-plus"></i>Tambah
									</button>
								</td>
							</tr>
...
<script>
	const state={
		...
		b4_e:[],
		b4_del:[],
		b4d_k6_total:'<?php echo $sub['b4d_k6_total']?>',
		b4e_k6_total:'<?php echo $sub['b4e_k6_total']?>',
	}
	...
	$(document).ready(()=>{
		...
		$('#b4d_k6_total').on('change',()=>{
			state.b4d_k6_total=`${$('#b4d_k6_total').val()}`
		})
		$('#b4e_k6_total').on('change',()=>{
			state.b4e_k6_total=`${$('#b4e_k6_total').val()}`
		})

		actionTambahPengeluaran('d')
		actionTambahPengeluaran('e')
