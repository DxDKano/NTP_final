...
<main id="main" class="main">
	...
	<section class="section">
		<form id="blok1Form" action="<?php echo base_url('PerikananTangkap/submitblok1')?>" class="row g-3">
		<div class="row">
				...
				<div class="card">
					<div class="card-body">
						...
						<table class="table table-sm table-bordered small">
							...
							<tr>
								<th>8. Jenis Penangkapan Utama</th>
								<td>
									<div class="d-flex justify-content-between">
										<div>1. Laut</div>
										<div>2. Perairan Umum</div>
									</div>
								</td>
								<td id="val_i8"><input id="b1_r8" name="b1_r8" type="number" class="form-control align-right" placeholder="" aria-label="b1_r8" value="<?php echo $sub['b1_r8']?>" required></td>
							</tr>
							<tr>
								<th>9. Jika Rincian 8 berkode 2, maka Jenis Perairan Umum Utama</th>
								<td>
									<div class="d-flex justify-content-between">
										<div>1. Danau</div>
										<div>2. Sungai</div>
								...
								<td id="val_i9"><input id="b1_r9" name="b1_r9" type="number" class="form-control align-right" placeholder="" aria-label="b1_r9" value="<?php echo $sub['b1_r9']?>" ></td>
							</tr>
							</tbody>
						...
						<div id="footernotif" class="text-center">
							<button id="submitBtn" type="button" class="btn btn-primary" data-url="<?php echo base_url("PerikananTangkap/halaman2"
									."?tahun={$sub['tahun']}"
									."&id_prov={$sub['id_prov']}"
									."&id_kab={$sub['id_kab']}"
									."&id_kec={$sub['id_kec']}"
...
<script>
	...
	const state = {
		...
		b1_r8:'<?php echo $sub['b1_r8'] ?>',
		b1_r9:'<?php echo $sub['b1_r9'] ?>',
		b2:{
	...
	$(document).ready(()=>{
		...
		// })
		$('#b1_r8').on('change',()=>{
			state.b1_r8=`${$('#b1_r8').val()}`
		})
		$('#b1_r9').on('change',()=>{
			state.b1_r9=`${$('#b1_r8').val()}`
		})
		...
		$('#submitBtn, .halaman-tabs').on('click',function (e) {
			...
			setTimeout(function (){
				$.ajax({
					url: `<?php echo base_url('PerikananTangkap/submithalaman1')?>`,
					type: 'POST',
					...
					success: function(response) {
						...
						if(response.ok){
							...
							// ?tahun=2022&id_prov=71&id_kab=07&id_kec=010&id_desa=001&no_ruta=34
							console.log(url)
							location.href=url
						}else {
							$('#footernotif').after(`
