				...
				<div class="card">
					<div class="card-body">
						...
						<div id="footernotif" class="text-center">
							<button id="backBtn" type="button" class="btn btn-secondary" data-url="<?php echo base_url("Peternakan/halaman9"
									."?tahun={$sub['tahun']}"
									."&id_prov={$sub['id_prov']}"
									."&id_kab={$sub['id_kab']}"
									."&id_kec={$sub['id_kec']}"
...
<script>
	...
	$(document).ready(()=>{
		...
		$('#submitBtn, #backBtn, .halaman-tabs').on('click',function (e) {
			...
			setTimeout(function (){
				$.ajax({
					url: `<?php echo base_url('Peternakan/submithalaman10')?>`,
					type: 'POST',
					...
					success: function(response) {
						...
						$('#submitBtn, #backBtn').empty();
						$('#backBtn').append(`<i class="bi bi-arrow-left"></i> Halaman 9 `);
