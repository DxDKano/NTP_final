...
<main id="main" class="main">
	...
	<section class="section">
		<form id="blok1Form" action="<?php echo base_url('Konsumsi/submitblok1')?>" class="row g-3">
		<div class="row">
				...
				<div class="card">
					<div class="card-body">
						...
						<table class="table table-sm table-bordered small">
							...
							<tr>
								<th>9.a. Jenis Komoditas Utama<br>&emsp;&emsp;&emsp;&emsp;&emsp;&emsp;(Jika rincian 8 selain perikanan tangkap)</th>
								<td>
									<!--<select class="form-select" id="i8" aria-label="i8" required>
										<?php foreach ($komoditas as $item): ?>
											<option value="<?php echo $item['id_komoditas'] ?>"
													<?php echo $sub["b1_9a"]==$item['id_komoditas']?"selected":"" ?>>
												<?php echo $item['komoditas'] ?>
											</option>
										<?php endforeach; ?>
									</select>-->
								</td>
								<td id="val_9a"><input id="b1_9a" type="number" class="form-control align-right" placeholder="Jenis Komoditas Utama" value="<?php echo $sub['b1_9a']?>" required></td>
							</tr>
							<tr>
								<th>9.b. Jenis Perahu Utama<br>&emsp;&emsp;&emsp;&emsp;&emsp;&emsp;(Jika rincian 8 perikanan tangkap)</th>
								<td>
									<table>
										<tr>
											<td>Kapal Motor</td>
								...
								<td id="val_9b"><input id="b1_9b" type="number" class="form-control align-right" placeholder=" Jenis Perahu Utama" value="<?php echo $sub['b1_9b']?>" required></td>
							</tr>
							</tbody>
						...
						<div id="footernotif" class="text-center">
							<button id="submitBtn" type="button" class="btn btn-primary" data-url="<?php echo base_url("Konsumsi/halaman2"
									."?tahun={$sub['tahun']}"
									."&id_prov={$sub['id_prov']}"
									."&id_kab={$sub['id_kab']}"
									."&id_kec={$sub['id_kec']}"
...
<script>
	...
	const state = {
		kode_petugas:'<?php echo $user['kode_petugas']  ?>',
		...
		b1_9a:'<?php echo $sub['b1_9a'] ?>',
		b1_9b:'<?php echo $sub['b1_9b'] ?>',
		b2:{
	...
	$(document).ready(()=>{
		$('#i3').on('change',()=>{
			...
			setTimeout(function (){
				$.ajax({
					type: "POST",
					url: `<?php echo base_url('Kuesioner/ambilDesa')?>`,
					data: data,
					contentType: false,
					processData: false,
					cache: false,
		...
		$('#b1_9a').on('change',()=>{
			state.b1_9a=`${$('#b1_9a').val()}`
			//$('#val_9a').html(state.b1_9a)
		})
		$('#b1_9b').on('change',()=>{
			state.b1_9b=`${$('#b1_9b').val()}`
			//$('#val_9b').html(state.b1_9b)
		})
		$('#b2_1_a').on('change',()=>{
		...
		$('#submitBtn, .halaman-tabs').on('click',function (e) {
			...
			setTimeout(function (){
				$.ajax({
					url: `<?php echo base_url('Konsumsi/submithalaman1')?>`,
					type: 'POST',
